...
bool Tracking::needNewKeyFrame() {
    //Check for ORB-SLAM 1 decisions

    //1) more than 20 frames from last relocalization
    // std::cout << "ALOHA" << std::endl;

    //2) inactive local mapping or more than 20 frames from last KF
    ...
    // //3) Current frame need to track at least 50 points
    // std::cout << "SAJDASND" << nFeatTracked_ << std::endl;
    if (nFeatTracked_ < 30)
    {
    ...
    // //4) Number of current points tracked must be at least the 90% of the previous frame (reference keyframe)
    // int nRefTracked = pLastKeyFrame_ ? pLastKeyFrame_->getNummberOfMapPoints() : 0;
    // if (nFeatTracked_ > 0.7 * nRefTracked)
    // {
