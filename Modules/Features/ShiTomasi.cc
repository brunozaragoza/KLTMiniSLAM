 ...
 */
 ...
 void ShiTomasi::extract(const cv::Mat &im, std::vector<cv::KeyPoint> &vKeys) {
     ...
     if(vKeys.empty()){
         ...
         vector<cv::KeyPoint> keypoints_already_extracted = vKeys;
         vKeys.clear();
 
         GetKeyPoints(vKeys, keypoints_already_extracted);
     }
 }
