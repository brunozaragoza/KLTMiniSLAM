 ...
 */
 ...
 public:
     ...
     void computeError()  {
         ...
         //Tcw is the transformation from world to camera 
         //to get a point from world to the camera frame we need to do Tcw*p3Dw
         Eigen::Vector3d p3Dc = Tcw.map(p3Dw);   //Predicted 3D camera position of the point
 
         //Project the 3D point into the image plane
         Eigen::Vector2d obsPredicted = pCamera->project(p3Dc);
 
         //Compute the error as the difference between the predicted and the observed position
         _error = obs - obsPredicted;
 ...
 public:
     ...
     void computeError()  {
         ...
         //Tcw is the transformation from world to camera
         //to get a point from world to the camera frame we need to do Tcw*p3Dw
         Eigen::Vector3d p3Dc = Tcw.map(Xworld);   //Predicted 3D camera position of the point
