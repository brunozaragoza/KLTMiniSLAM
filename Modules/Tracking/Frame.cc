...
void Frame::setKeyPoints(std::vector<cv::KeyPoint> &pts)
{
    // Copy keypoints
    vKeys_ = std::move(pts); // transfers ownership
}
void Frame::setLandmarkStatuses(const std::vector<LandmarkStatus> input)
{
    landmarkstatuses_ = input;
}
...
std::vector<cv::KeyPoint> Frame::GetKeypointsWithStatus(
    const std::vector<LandmarkStatus> statuses) const
{
    ...
    for (int idx = 0; idx < landmarkstatuses_.size(); idx++)
    {
        ...
        {
            keypoints.push_back(vKeys_[idx]);
        }
    }
